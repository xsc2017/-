...
dim SortID, LanguageID, ParentID, SortOrder, SortType, SortName, SortURL, SortLevel, AddTime, PageTitle, PageKeywords, PageDesc, SortPath, SortTemplate, ContentTemplate, SortFolder, ContentFolder, SortFileName, ContentFileName, SortStatus, TopSortID, GroupID, Exclusive, Content,IndexImage,ImagePath
dim sql, msg
...
Sub aboutList
	Dim rsObj	:	Set rsObj=conn.Exec("select * from {prefix}Sort where SortType=1 and LanguageID="&session("languageID")&"  Order by SortOrder Asc,SortID","r1")
	If rsObj.Eof Then 
...
Sub editSort	
	...
	conn.exec "update {prefix}Sort set GroupID="&GroupID&", SortName='"&SortName&"', PageTitle='"&PageTitle&"', PageKeywords='"&PageKeywords&"', PageDesc='"&PageDesc&"', SortContent='"&Content&"', Exclusive='"&Exclusive&"',IndexImage='"&IndexImage&"',ImagePath='"&ImagePath&"' where SortID="&SortID, "exe"	
	alertMsgAndGo "修改成功","AspCms_aboutedit_user.asp?id="&SortID
End Sub
%>