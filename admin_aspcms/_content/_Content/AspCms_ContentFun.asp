...
case 2
         ...
         echo "<script type=""text/javascript"">var editor"&cn&" = new UE.ui.Editor();editor"&cn&".render("""&cn&""");</script>"
case 3
...
Dim sql
	...
	if not isdate(AddTime) then AddTime=formatDate(now(),4)
	if not isdate(Timeing) then Timeing=formatDate(now(),4)
...
Sub carryContent
	dim id : id=getForm("id","post")
	if isnul(id) then alertMsgAndGo "请选择要操作的内容","-1"
	dim carrySortID,i,sql,specStr
	if isnul(getForm("carrySortID","post")) then alertMsgAndGo "请选择其它语言的具体分类！","-1"
	carrySortID=split(getForm("carrySortID","post"),"_")
	...
	for i=0 to ubound(copyid)
		conn.exec "insert into {prefix}Content (VideoGroupID,DownGroupID, LanguageID,  GroupID, Exclusive, TitleColor, Title, Title2, Author, ContentSource, [Content], ContentTag, PageTitle, PageKeywords, PageDesc, OutLink, IsOutLink, ContentStatus, IsTop, IsRecommend, IsImageNews, IsHeadline, IsFeatured, IsGenerated, ContentOrder, Visits, AddTime, EditTime, ImagePath, IndexImage, DownURL, PageFileName, IsNoComment,TimeStatus,Timeing, Star"&specStr&",SortID) select VideoGroupID, DownGroupID, "&carrySortID(0)&", GroupID, Exclusive, TitleColor, Title, Title2, Author, ContentSource, [Content], ContentTag, PageTitle, PageKeywords, PageDesc, OutLink, IsOutLink, ContentStatus, IsTop, IsRecommend, IsImageNews, IsHeadline, IsFeatured, IsGenerated, ContentOrder, Visits, AddTime, EditTime, ImagePath, IndexImage, DownURL, PageFileName, IsNoComment,TimeStatus,Timeing, Star"&specStr&","&carrySortID(1)&" from {prefix}Content where contentid="&copyid(i)&"", "exe"	
	next
	
	alertMsgAndGo "传送成功！", getPageName()&"?sortType="&sortType&"&sortid="&sortid&"&keyword="&keyword&"&page="&page&"&psize="&psize&"&order="&order&"&ordsc="&ordsc	
End Sub
	...
	for each tag in tags
		if not isnul(tag) then 	
			...
			if not rs.eof then 
				...
				sql="insert into {prefix}Tag(TagName, TagCount, SortType, SortID, TagVisits, LanguageID, AddTime) values('"&tag&"',0,"&SortType&","&SortID&",0,"&LanguageID&",'"&formatDate(now(),4)&"')"
				conn.exec sql,"exe"
...
Function languageList
	dim rss
	set rss=conn.exec("select LanguageID, LanguageName from {prefix}Language where LanguageStatus=1 order by LanguageOrder ,LanguageID", "exe")
	do while not rss.eof 
		if cint(session("languageID"))<>rss(0) then
			echo "<optgroup label="&rss(1)&">"
			languageSortList rss(0),0
			echo "</optgroup>"
		end if
		rss.moveNext
	loop
...
'获取语言下分类列表
Function languageSortList(Byval languageId,Byval ParentID)
	dim rs,sel
	sel=""
	set rs =conn.Exec ("select SortID,SortName,ParentID,SortLevel,(select count(*) from {prefix}Sort where ParentID=t.SortID) as c from {prefix}Sort as t where SortType<>1 and LanguageID="&languageId&" and ParentID="&ParentID,"r1")
	Do While Not rs.Eof	
		dim rscount:rscount=0		
		echo "<option value="""&languageId&"_"& rs(0) &""" "&sel&">"&getLevel_(rs(3))&rs(1) &"</option>" & vbcr
		if rs(4)>0 then 
		    languageSortList=languageSortList(languageId,rs(0))
		end if
		rs.MoveNext
	Loop
