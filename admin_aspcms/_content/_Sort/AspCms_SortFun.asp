...
dim SortID, LanguageID, ParentID, SortOrder, SortType, SortName, SortURL, SortLevel, AddTime, PageTitle, PageKeywords, PageDesc, SortPath, SortTemplate, ContentTemplate, SortFolder, ContentFolder, SortFileName, ContentFileName, SortStatus, TopSortID, GroupID, Exclusive, Content,indeximage,ImagePath,IcoImage
dim sql, msg,SortEnName,usertype
Sub getSort
	dim id : id=getForm("id","get")
	usertype=getForm("usertype","both")
...
Sub quickAdd
	...
	for i=1 to 10	
		...
		if not isnul(TopSortName) then 
			...
			conn.exec "insert into {prefix}Sort(ParentID, SortOrder, SortType, SortName, SortURL, SortTemplate, ContentTemplate, SortFolder, ContentFolder, SortFileName, ContentFileName, SortStatus, LanguageID, AddTime, GroupID, Exclusive,ImagePath,IcoImage) values("&ParentID&", "&SortOrder&", "&SortType&", '"&TopSortName&"', '"&SortURL&"', '"&SortTemplate&"', '"&ContentTemplate&"', '"&SortFolder&"', '"&ContentFolder&"', '"&SortFileName&"', '"&ContentFileName&"', "&SortStatus&", "&LanguageID&", '"&AddTime&"', "&GroupID&", '"&Exclusive&"','"&ImagePath&"','"&IcoImage&"')", "exe"
			SortID=Conn.Exec("select @@identity","r1")(0)
			SortLevel="1"
			TopSortID=SortID
			SortPath = TopSortID&","			
			conn.exec "update {prefix}Sort set SortLevel="&SortLevel&", TopSortID="&TopSortID&", SortPath='"&SortPath&"', ImagePath='"&ImagePath&", IcoImage='"&IcoImage&" where SortID="&SortID, "exe"
...
Sub saveAll
	...
	Dim SortEnName  	: 	SortEnName=split(getForm("SortEnName","post"),",")   
...
If Ubound(SortEnName)=-1 Then   'by 暗辰
  ReDim SortEnName(0)       '防止有值为空时下标越界
  SortEnName(0)=""
 End If                  'by 暗辰
	...
	For i=0 To Ubound(ids)		
		...
		if SwitchNavEn<>0 then
		Conn.Exec "update {prefix}Sort Set SortName='"&trim(SortNames(i))&"',SortEnName='"&trim(SortEnName(i))&"',SortURL='"&trim(SortURLs(i))&"',SortOrder='"&trim(SortOrders(i))&"' Where SortID="&trim(ids(i)),"exe"	
		else
...
function sortList(ParentID)
	Dim rs :set rs =Conn.Exec ("select *,(select count(*) from {prefix}Sort where ParentID=t.SortID) as c from AspCms_Sort t where LanguageID="&session("languageID")&" and ParentID="&ParentID&" order by Sortorder ","r1")
	...
	Else
		...
		Do While not rs.eof 
			...
			echo "<td align=""left"" width=280>"&getLevel(rs("SortLevel"))&showtype&"<input name=""SortNames"" type=""text"" class=""input"" id=""SortNames"" value="""&rs("SortName")&""" maxlength=""200"" style=""width:80px;""/>"
			if SwitchNavEn=1 then
			echo "-<input name=""SortEnName"" type=""text"" class=""input"" id=""SortEnName"" value="""&rs("SortEnName")&""" maxlength=""200"" style=""width:80px;""/></td>"&vbcrlf
			end if
			echo "<td>"	
...
Sub addSort	
	...
	SortEnName = getForm("SortEnName","post")
...
Sub editSort	
	...
	conn.exec "update {prefix}Sort set ParentID="&ParentID&",  SortOrder="&SortOrder&", SortType="&SortType&", SortStatus="&SortStatus&", LanguageID="&LanguageID&", GroupID="&GroupID&", SortName='"&SortName&"', SortEnName='"&SortEnName&"', SortURL='"&SortURL&"', PageTitle='"&PageTitle&"', PageKeywords='"&PageKeywords&"', PageDesc='"&PageDesc&"', SortTemplate='"&SortTemplate&"', ContentTemplate='"&ContentTemplate&"', SortFolder='"&SortFolder&"', ContentFolder='"&ContentFolder&"', SortFileName='"&SortFileName&"', ContentFileName='"&ContentFileName&"', SortContent='"&Content&"',AddTime ='"&AddTime&"', Exclusive='"&Exclusive&"',indeximage='"&indeximage&"',IcoImage='"&IcoImage&"',ImagePath='"&ImagePath&"' where SortID="&SortID, "exe"
	...
	conn.exec "update {prefix}Sort set SortLevel="&SortLevel&", TopSortID="&TopSortID&", SortPath='"&SortPath&"' where SortID="&SortID, "exe"	
	editSubSort(SortID)
	if not isnul(usertype) then
	alertMsgAndGo "修改成功","AspCms_SortEdit.asp?id="&SortID&"&usertype=1"
	else
...
Sub delSort
	...
	if switchSortDefend=1 then exit sub
		'删除子分类和子类中的内容		
