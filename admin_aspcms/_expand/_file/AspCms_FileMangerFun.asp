...
dim path,fileListArray,i,fileAttr,folderListArray,folderAttr
	...
	if ( not instr(path,"upLoad")>0 ) or instr(path,"..")>0 then	
		echo "<tr bgcolor=""#FFFFFF"" align=""center""><td colspan=""7"" algin=""center"">你无权限访问些目录,<a href=""javascript:history.go(-1)"" class=""txt_C1"">返回</a></td></tr>"
		response.end
	end if
...
Function getAllFileList(path,countnum)
	...
	dim rsObj,rsObj1 ,rsObj2,rsObj3,rsObj4,rsObj5
	fileListArray= getFileList(path)
	if instr(fileListArray(0),",")>0 then	
		for  i = 0 to ubound(fileListArray)
			fileAttr=split(fileListArray(i),",")
			...
			Set rsObj=Conn.Exec("select count(*) from {prefix}Content where IndexImage like '%"&finame&"%' or ImagePath like '%"&finame&"%' or DownURL like '%"&finame&"%' or Content like '%"&finame&"%' or ContentSource like '%"&finame&"%' ","r1")'By XingMeng修改
			...
			Set rsObj4=Conn.Exec("select count(*) from {prefix}Sort where SortContent like '%"&finame&"%' or ImagePath like '%"&finame&"%' or IndexImage like '%"&finame&"%' or IcoImage like '%"&finame&"%' or  IcoPath like '%"&finame&"%'  ","r1")  'By XingMeng修改
			Set rsObj5=Conn.Exec("select count(*) from {prefix}Labels where LabelContent like '%"&finame&"%' ","r1") 'By XingMeng添加

			if not rsObj(0)>0 and not rsObj1(0)>0 and not rsObj2(0)>0 and not rsObj3(0)>0 and not rsObj4(0)>0  and not rsObj5(0)>0 and not instr(lcase(slideImgs),lcase(finame))>0 and not instr(lcase(slideImgsB),lcase(finame))>0 and not instr(lcase(slideImgsC),lcase(finame))>0 and not instr(lcase(slideImgsD),lcase(finame))>0 then
			 echo "<tr bgcolor=""#ffffff"" align=""center"" onMouseOver=""this.bgColor='#CDE6FF'"" onMouseOut=""this.bgColor='#FFFFFF'"">"&vbcrlf& _
