...
Sub plugadd
	...
	sql="insert into {prefix}Menu(ParentID, TopID, MenuName, MenuLink, MenuOrder, MenuLevel, MenuStatus,Menukey) values(52, 52, '"& Menunames(i)&"', '"&path & "/"& plugpath&"/"&MenuLinks(i)&"', 1, 2, 1,'"&plugkey&"')"
	conn.exec sql,"exe"
	next
	...
	pluginInstall
	echo "<"&"script language='javascript'>alert('安装成功');window.parent.frames['topFrame'].location = '../top.asp?'+Math.random();if(window.navigator.userAgent.indexOf(""MSIE"")>=1){top.document.frames.menu.location = '../menu.asp?id=52';}else if(window.navigator.userAgent.indexOf(""Firefox"")>=1){top.document.getElementById('menu').src ='menu.asp?id=52';}else{top.document.getElementById('menu').src ='menu.asp?id=52';}</"&"script>"
...
Sub pluginInstall
	...
	'f.Close
	'Set f = nothing
	s=loadFile("/plug"&"/"&path&"\#install.inc")
	Execute(s)	
