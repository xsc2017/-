...
Sub languageList
	...
	if rs.eof then 
		...
		do while not rs.eof 
			...
			"<TD><a href=""AspCms_LanguageEdit.asp?id="&rs(0)&""">修改</a> <a href=""?action=del&id="&rs(0)&""" onClick=""return confirm('确定要删除吗，这样会同时删除相应的内容，请核对再操作')"">删除</a></TD>"&vbcrlf& _
			"</TR>"&vbcrlf
			rs.moveNext
		loop
	end if	
...
Sub delLanguage
	...
	if id=session("languageID") then alertMsgAndGo "请不要在当前语言环境删除当前语种！","-1"
	if isnul(id) then alertMsgAndGo "请选择要操作的内容","-1"
	conn.exec "delete from {prefix}Language where IsDefault=0 and LanguageID in("&id&")","exe"
	conn.exec "delete from {prefix}Content where LanguageID in("&id&")","exe"
	conn.exec "delete from {prefix}Sort where LanguageID in("&id&")","exe"
