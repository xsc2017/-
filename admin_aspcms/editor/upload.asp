...
Dim Toimg : Toimg =getForm("Toimg", "get")
...
%>
<form name="form" enctype="multipart/form-data" action="?action=save&sortType=<%=sortType%>&stype=<%=stype%>&Tobj=<%=Tobj%>&toimg=<%=toimg%>"  method="post">
<input type="file" style=" width:250px;" class="input" name="filedata"/>
...
function uploadfile()
	...
	select case sortType
		...
		case "8"
		attachdir=sitePath&"/"&upLoadPath &"/video"		
		case "10"
		...
		attachdir=sitePath
		case "16"
		attachdir=sitePath&"/"&upLoadPath &"/weixin"
		case else
	...
	if stype <> "video" then
		...
		maxattachsize=10240000'最大上传大小，默认是10M
	end if
	...
	if upfile.isErr then
		...
		for each formName in upfile.file '列出所有上传了的文件
			...
			if oFile.FileExt="" then alertMsgAndGo "无数据提交","-1"		
			...
			if sorttype=15 or sorttype=16  then
...
<% 
elseif stype="video" then
echo "<script language=""javascript"">parent.document.getElementById(""videoPath"").value='"&target&"'</script>"
%><%
elseif stype="file" then
	echo "<script language=""javascript"">parent.document.getElementById("""&Tobj&""").value='"&target&"'</script>" 
	if toimg<>"" then echo "<script language=""javascript"">parent.document.getElementById("""&toimg&""").src='"&target&"'</script>" 							
...
Public Function isAllowExt(Ext)
    ...
    Else
        ...
        For Each a In ar
            If a <> "" Then
                If InStr(LCase(Ext), LCase(a)) > 0 Then
                    isAllowExt = False
                    Exit Function
                End If
            End If
