...
if session("GroupMenu")="all" then
	...
	sql="select MenuID, MenuName, MenuLink, (select Count(*) from {prefix}Menu as b where MenuStatus=1 and b.ParentID=a.MenuID ) from {prefix}Menu as a where MenuStatus=1 and ParentID=0 and MenuID in("&session("GroupMenu")&") order by MenuOrder"
end if
...
if session("GroupMenu")="all" then
	...
	sql="select MenuID, MenuName, MenuLink from {prefix}Menu where MenuStatus=1 and ParentID="&rs(0)&" and MenuID in("&session("GroupMenu")&")"&" order by MenuOrder"
end if
