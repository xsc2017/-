...
Sub addGbook
	...
	if not IsSafeStr(Contact) then alertMsgAndGo "您的联系人里包含了不安全字段，请重新输入","-1"
	if not isnul(ContactWay) then
		if not (CheckMobile(ContactWay) or CheckTelPhone(ContactWay)) then alertMsgAndGo "您输入的电话号码格式不正确，请重新输入","-1"
	end if
	...
	if isnul(Content) then alertMsgAndGo "内容不能为空","-1"
	if isnul(Contact) then alertMsgAndGo "联系人不能为空","-1"
	...
	if messageReminded then 
		dim n,mails:mails=split(messageAlertsEmail,",")
		for n=0 to ubound(mails)
			sendMail mails(n),setting.sitetitle,setting.siteTitle&setting.siteUrl&"--留言信息提醒邮件！","您的网站<a href=""http://"&setting.siteUrl&""">"&setting.siteTitle&"</a>有新的留言信息！<br>"&"<br>问题："&FaqTitle&"<br>内容："&Content&"<br>联系人："&Contact&"<br>联系方式："&ContactWay&"<br>时间："&AddTime	
		next
	end if
