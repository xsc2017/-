...
Function makeAbout(Byval sortID, Byval page, Byval isMakeHtml)
	...
	templateObj.content=replace(templateObj.content,"{aspcms:ensortname}",repnull(rsObj("SortEnName")))
	...
	if not isNul(rsObj("IcoImage")) then 
		if instr(rsObj("IcoImage"),"http://")>0 then 
			templateObj.content = replace(templateObj.content,"[about:IndexImage]",rsObj("IcoImage")) 'By XingMeng修改
		else
			templateObj.content = replace(templateObj.content,"[about:IndexImage]",rsObj("IcoImage")) 'By XingMeng修改
		end if
	else
...
function makeContent(Byval contentID, Byval page, Byval isMakeHtml)
	...
	set rsobj=conn.exec("select ContentID,a.SortID,a.GroupID as aGroupID ,a.Exclusive as aExclusive,b.GroupID as bGroupID,b.Exclusive as bExclusive,Title,Title2,TitleColor,IsOutLink,OutLink,Author,ContentSource,ContentTag,Content,ContentStatus,IsTop,Isrecommend,IsImageNews,IsHeadline,IsFeatured,ContentOrder,IsGenerated,Visits,a.AddTime,a.[ImagePath],a.IndexImage,a.DownURL,a.PageFileName,a.PageDesc,a.PageKeywords,SortType,SortURL,SortFolder,SortFileName,SortName,SortEnName,ContentFolder,ContentFileName,SortTemplate,ParentID,TopSortID,ContentTemplate,IsNoComment "&sperStr&",a.DownGroupID,a.VideoGroupID from {prefix}Content as a,{prefix}Sort as b where a.LanguageID="&setting.languageID&"and a.SortID=b.SortID and ContentStatus=1 and TimeStatus=0 and IsOutLink=0 and ContentID="&ContentID, "exe")							
	...
	if not isnul(rsObj("aGroupID")) then	
		if not ViewNoRight(rsObj("aGroupID"),rsObj("aExclusive")) or not ViewNoRight(rsObj("bGroupID"),rsObj("bExclusive")) then 
	...
	templateObj.parseNavList("")
	templateObj.parseLoop("content")
	...
	templateObj.content=replace(templateObj.content,"["&str&":sortlink]",templateObj.getSortLink(rsObj("sortType"),rsObj("sortID"),rsObj("sortUrl"),rsObj("sortFolder"),rsObj("sortFileName"),rsObj("bGroupID"),rsObj("bExclusive")))

	templateObj.content=replace(templateObj.content,"["&str&":date]",rsObj("Addtime"))
	
	'内容页面日期格式化 BY XingMeng
	dim regExpObj,matches,match,fieldNameArr,m,fieldArr,fieldName,timestyle
	set regExpObj= new RegExp
	regExpObj.ignoreCase=false
	regExpObj.Global=true
	regExpObj.Pattern =  "\["&str&":(date[\s\S]+?)\]"
	set matches=regExpObj.Execute(templateObj.content)
	for each match in matches
		fieldNameArr = trim(regExpReplace(match.SubMatches(0),"[\s]+",chr(32)))
		...
		if fieldArr<>"" then
			timestyle=replace(fieldArr,"style=","")
			templateObj.content=replace(templateObj.content,"["&str&":"&match.SubMatches(0)&"]",FormatDate(rsObj("Addtime"),timestyle))
		else
			templateObj.content=replace(templateObj.content,"["&str&":"&match.SubMatches(0)&"]",rsObj("Addtime"))
		end if
	next
	...
	contentLink=templateObj.getContentLink(rsObj("SortID"),rsObj("ContentID"),rsObj("SortFolder"),rsObj("aGroupID"),rsObj("ContentFolder"),rsObj("ContentFileName"),rsObj("AddTime"),rsobj("PageFileName"),rsObj("bGroupID"))
	templateObj.content=replace(templateObj.content,"["&str&":link]","http://"&setting.siteUrl&contentLink)
	templateObj.content=replace(templateObj.content,"["&str&":downurl]",sitePath&"/down/?"&rsObj("ContentID")&".html")
	if isnul(rsObj("PageKeywords")) then 
	...
	content=replacekey(decodeHtml(rsObj("Content")))	
	tempStr=templateObj.content	
	...
	if isExistStr(content,"{aspcms:page}") then
		...
		makeContent=makeContentImages(templateObj.content)
		if isMakeHtml then 
			createTextFile  makeContent, contentLink, "" :echo contentLink&"<br>"
		end if
	end if	
	rsObj.close : set rsObj=nothing	
	set templateobj=nothing 	
...
'if instr(sConetnt,"{aspcms:cimages") then
	...
	for each match in matches
		...
		m_type = parseArr(match.SubMatches(0))("type")
		
		if isnul(m_maxcount) or not isnumeric(m_maxcount) then m_maxcount = 9999
				
		if m_type="about" then  
			sql="select imagepath from {prefix}Sort where SortID="
		else
			sql="select imagepath from {prefix}Content where ContentID="
		end if
...
Function strList(Byval sortID, Byval page)
	...
	if isnul(rsObj("SortContent")) then 
		...
		templateObj.content=replace(templateObj.content,"{aspcms:SortContent}",decodeHtml(rsObj("SortContent")))
	end if
	
	if not isNul(rsObj("IcoImage")) then 
		if instr(rsObj("IcoImage"),"http://")>0 then 
			templateObj.content = replace(templateObj.content,"{aspcms:indeximage}",rsObj("IcoImage"))
		else
			templateObj.content = replace(templateObj.content,"{aspcms:indeximage}",rsObj("IcoImage"))
		end if
	else
	...
	if setting.htmlFilePath="wap" then
		if sitePath<>"" then
