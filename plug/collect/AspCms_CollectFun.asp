...
Sub addContent
	...
	sql="insert into {prefix}Content(Title,[Content],LanguageID, SortID, GroupID, Exclusive, TitleColor, ContentStatus, IsTop, IsRecommend, IsImageNews, IsHeadline, IsFeatured, IsGenerated, ContentOrder, Visits,AddTime,IsOutlink) select C_Title,C_Content,"&LanguageID&","& SortID &",2,'>=','#000000',1,0,0,0,0,0,0,0,0,'"& formatDate(now(),4) &"',0 from {prefix}Collect_Content where C_ContentID in("&id&",)"
	conn.exec sql,"exe"
	if isdel="del" then
...
Sub contentsave
	...
	for i=ubound(arrtc) to 1 step -1
		if issame(i,ids)=true then
			...
			sql="insert into {prefix}Collect_Content(C_Title,C_Content,AddTime) values('"&Tcs(0)&"', '"&replace(Tcs(1),"'","&qpos;")&"', '"&formatDate(now(),4)&"')"
			conn.exec sql,"exe"	
		end if
	next
	session("savecontent")=""
...
Sub collectsave
	dim Crules
		urlb=filterPara(getForm("urlb","post"))
		pagenum=filterPara(getForm("pagenum","post"))
		urla=filterPara(getForm("urla","post"))
		pagelinkcount=filterPara(getform("pagelinkcount","post"))
		listpageareab=filterPara(getForm("listpageareab","post"))
		listpageareaa=filterPara(getForm("listpageareaa","post"))
		listtitleb=filterPara(getForm("listtitleb","post"))
		listtitlea=filterPara(getForm("listtitlea","post"))
		pagelinknum=filterPara(getForm("pagelinknum","post"))
		purl=filterPara(getForm("purl","post"))
		articletitb=filterPara(getForm("articletitb","post"))
		articletita=filterPara(getForm("articletita","post"))
		articleconb=filterPara(getForm("articleconb","post"))
		articlecona=filterPara(getForm("articlecona","post"))
		contentnum=filterPara(getForm("contentnum","post"))
		rulesname=filterPara(getForm("rulesname","post"))
		CollectID=filterPara(getForm("DocollectID","post"))
	...
	if isNul(CollectID) then 
		sql="insert into {prefix}Collect(CollectName,CollectRules,Addtime) values('"&rulesname&"', '"&Crules&"', '"&formatDate(now(),4)&"')"
	else
	...
	for i=1 to ubound(arrlinkvalue)
		if issame(i,ids)=true or todo="this" then
		   ...
		   echo "<TR class=list>"&vbcrlf& _
				...
				"<TD colspan=3>"&getStrByLen(filterPara(Ccontent),500)&"</TD></TR>"
		end if
	next
